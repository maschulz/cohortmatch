...
against R's MatchIt, and handles biobank-scale cohorts (500k rows).
...
Not yet on PyPI. The `viz` extra adds plotting and HTML reports.

## Quick start

Runnable as-is, `load_lalonde()` ships with the package:
...
result.matched_data              # matched units, original index preserved
result.balance()                 # signed SMD per covariate, before and after
result.pairs                     # treatment_id, control_id, distance, match_group

result.estimate_effects("re78")  # weighted effect with cluster-robust SE
result.supplement("supp.md")     # one-call methods+results record for a paper
```
